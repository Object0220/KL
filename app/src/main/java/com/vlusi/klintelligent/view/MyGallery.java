...
public class MyGallery extends Gallery {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (view instanceof MyImageView) {
            ...
            if ((int) width <= ImageMainActivity.screenWidth && (int) height <= ImageMainActivity.screenHeight)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
            {
...
//				}
                ...
                if (distanceX > 0)// 向左滑动
                {
                    if (r.left > 0) {// 判断当前ImageView是否显示完全
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else if (right <= ImageMainActivity.screenWidth) {
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                        imageView.postTranslate(-distanceX, -distanceY);
                    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if (view instanceof MyImageView) {
                    ...
                    if ((int) width <= ImageMainActivity.screenWidth && (int) height <= ImageMainActivity.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
                    {
                        break;
                    }
                    float v[] = new float[9];
                    ...
                    if (bottom < ImageMainActivity.screenHeight) {
                        imageView.postTranslateDur(ImageMainActivity.screenHeight - bottom, 200f);
                    }
                }
                break;
        }
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (view instanceof MyImageView) {
                ...
                if (imageView.getScale() > imageView.getScaleRate()) {
                    imageView.zoomTo(imageView.getScaleRate(), ImageMainActivity.screenWidth / 2, ImageMainActivity.screenHeight / 2, 200f);
                    // imageView.layoutToCenter();
                } else {
                    imageView.zoomTo(1.0f, ImageMainActivity.screenWidth / 2, ImageMainActivity.screenHeight / 2, 200f);
                }
            } else {
            }
